...
class FullyFusedMLP(nn.Module):
    ...
    def __init__(
        ...
        # Hidden
        if depth > 1:
            layer = nn.ModuleList([nn.Linear(d_hidden, d_hidden) for _ in range(depth)])
            self.w_hid = torch.cat([layer.weight for layer in layer], 0)
            self.b_hid = torch.stack([layer.bias for layer in layer], 0)
        else:
